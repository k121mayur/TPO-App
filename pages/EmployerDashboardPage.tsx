...
import { Plus, Edit, Trash2, ShieldCheck, Briefcase, Eye } from 'lucide-react';
...
const EmployerDashboardPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      const [employerJobs, companyData] = await Promise.all([getEmployerJobs(), companyPromise]);
  ...
  if (!company) {
    return (
      <div className="flex justify-center items-center h-screen text-center">
        <div>
          <p className="text-lg font-semibold text-gray-700">Company data is unavailable.</p>
          <p className="mt-2 text-gray-500">Please reach out to support so we can verify your organization.</p>
        </div>
      </div>
    );
  }
            ...
            <div className="space-y-4">
              {jobs.length > 0 ? (
                jobs.map((job) => (
                  <div
                    key={job.id}
                    className="p-4 border rounded-lg flex flex-col md:flex-row justify-between items-start md:items-center hover:bg-gray-50 transition-colors"
                  >
                    <div>
                    ...
                    <div className="flex items-center gap-2 mt-4 md:mt-0">
                      ...
                      <button className="p-2 text-gray-500 hover:text-green-600 hover:bg-green-100 rounded-full">
                        <Edit size={18} />
                      </button>
